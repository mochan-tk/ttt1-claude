...
| A rule existed only in someone's head or a review thread | `.claude/rules/*.md` or `AGENTS.md` (mind the tier — see `context-distillation` §Tiering) |
| A multi-step procedure was improvised inconsistently | a skill under `.claude/skills/` (new or amended) |
...
3. Apply the **Budget rule** for always-on files (`AGENTS.md`,
   `CLAUDE.md`): they must stay lean (target well under ~150
...
Fix cell with `[upstreamed]`. Project tunings (Sync Pair content, globs,
