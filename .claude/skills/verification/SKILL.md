...
3. **AI review** — the `reviewer` subagent (`.claude/agents/reviewer.md`),
   guided by `.claude/rules/code-review.md`, plus any platform review bots. Catches claim/evidence
...
(CLAUDE.md, rejection list). Sanitized excerpts are fine when
...
- **Environment** (missing tool, network, flake): fix the environment —
  usually CI config — and note it as a retro candidate.
