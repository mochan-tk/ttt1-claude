...
argument-hint: [lookback]
---

Follow `.claude/skills/retro/SKILL.md`.

Lookback window: $ARGUMENTS (e.g., "last 20 merged PRs" or "last 2 weeks")
...
3. For the top 1–3 classes, classify the root cause and pick the most
   deterministic asset that can host the fix (CI check > rule > skill), per
   the skill's table.
