...
|---|---|---|---|
| `exec:cloud` | Claude Code web/cloud session (remote environment) | Fully async, parallel at scale, ephemeral clean env, delivers a draft PR, iterates on CI failures | Brief is self-contained and unambiguous; no local/hardware needs; ideal for tests, refactors, docs, well-specified features |
| `exec:app` | Claude Code desktop app / orchestrator session (subagents, worktrees) | Steerable in real time, parent/child structure for orchestration, per-session model/agent choice, local checkout | Orchestration itself; tasks needing occasional steering; parallel local work isolated by worktrees; when model choice matters per task |
| `exec:cli` | Claude Code CLI (interactive or headless `-p`) | Scriptable, composable with `gh`, runs in CI/automation | Batch/repetitive repo operations, plan-graph manipulation at scale, scheduled or pipeline-triggered agent work |
| `exec:ide` | Claude Code IDE extension (human in the loop) | Human-in-the-loop, full local toolchain, hardware access (e.g., PlatformIO upload/monitor) | Ambiguous or exploratory work; design spikes; anything touching physical devices |
...
Suggest a role when a specialized definition exists in `.claude/agents/`
